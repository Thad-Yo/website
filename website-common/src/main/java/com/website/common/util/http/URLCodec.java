...
public class URLCodec {
    ...
    public static String decodeURL(String encodedURLString, String charsetName, boolean unescapeSpaces) {
        ...
        /*
         * if (unescapeSpaces) { encodedURLString = StringUtil.replace(
    ...
    private static CharBuffer _getRawCharBuffer(String rawString, int start, boolean includeSpaces) {
        ...
        for (int i = start; i < rawString.length(); i++) {
            ...
            if (!_validChars.get(rawChar) && ((rawChar != CharPool.SPACE) || includeSpaces)) {
    ...
    private static Log _log = LogFactory.getLog(URLCodec.class);
