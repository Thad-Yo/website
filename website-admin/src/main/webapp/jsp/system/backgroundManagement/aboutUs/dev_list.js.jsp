...
var ActionEvents = {
        ...
        actionFormatter: function (value, row) {
            var content = [];
            <shiro:hasPermission name="dev:EDIT">
            content.push('<button type="button" title="编辑" class="btn btn-primary btn-xs list-btn fn-edit">编辑</button>');
            </shiro:hasPermission>
            if(row.hasEng=="1"){
            	<shiro:hasPermission name="dev:EDIT_ENG">
            	content.push('<button type="button" title="编辑英文" class="btn btn-warning btn-xs list-btn fn-edit-en">编辑英文版</button>');
	            </shiro:hasPermission>            
            }else{
            	<shiro:hasPermission name="dev:EDIT_ENG">
            	content.push('<button type="button" title="添加英文" class="btn btn-warning btn-xs list-btn fn-edit-en">添加英文版</button>');
            	 </shiro:hasPermission>
            }
            <shiro:hasPermission name="dev:DELET">
	...
	Events = {
		addAction:function (selects,flag) {
            layerIndex = layer.open({
                ...
                area: ['800px', '400px'],
                maxmin: true,
                content: [webRoot + '/system/backgroundManageMent/aboutUsController/addInfo?']
            });
		...
		editAction:function (selects,flag) {
            layerIndex = layer.open({
                ...
                area: ['800px', '400px'],
                maxmin: true,
                content: [webRoot + '/system/backgroundManageMent/aboutUsController/editInfo?id='+selects[0].id+"&isEng="+flag]
            });
            layer.full(layerIndex);
        },
